#! /usr/bin/python3
'''
qucs_netlist.py License: GPL-3+ Chris Rogers 31st March 2020
...
parts reading problem
	do some tests
	if they fail copy it into work directory
	can use a . file in Windows or Mac?
'''
...
namebrowser = 'qucs_netlist.html'
namehelp = 'qucs_netlist.hlp'
nameparts = 'qucs_netlist.dat'


print('For more detailed help, a list of convertable parts or example files, see https://github.com/PopoutApps/qucs_netlist')

# link to package filenames
...
if len(sys.argv) < 2:
	...
	#filesim = '.'+ filename + '.sim' # hide the intermediate .sim file
...
# check for a valid parts file, load if ok
if os.path.isfile(fileparts):
